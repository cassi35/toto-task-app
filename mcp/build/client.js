...
async function main() {
    ...
    const [{ resources }, { prompts }, { tools }, { resourceTemplates }] = await Promise.all([
        mcp.listResources(),
        mcp.listPrompts(),
        mcp.listTools(),
        ...
        switch (option) {
            case 'Tools':
                const toolName = await select({
                    ...
                    choices: tools.map((tool) => tool.description
                        ? {
            ...
            case 'Resources':
                const resourceUri = await select({
                    ...
                    choices: [
                        ...resources.map((resource) => resource.description
                            ? {
                                name: resource.name,
                                value: resource.uri,
                        ...
                        ...resourceTemplates.map((template) => template.description
                            ? {
                                name: template.name,
            ...
            case 'Prompts':
                const promptName = await select({
                    ...
                    choices: prompts.map((p) => p.description
...
async function handleQuery(tools) {
    ...
    const aiTools = tools.reduce((acc, mcpTool) => {
        acc[mcpTool.name] = {
    ...
    console.log(text || JSON.stringify(toolResults[0]?.output) || 'No text generated.');
}
