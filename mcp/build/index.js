...
server.resource('user-details', new ResourceTemplate('users://{userId}', { list: undefined }), {
...
server.tool('create-ramdom-user', 'create a random user in the database', {
    ...
    try {
        ...
        fakeUser = JSON.parse(result.content.text
            .trim()
            .replace(/^```json\s*/i, '')
            .replace(/\s*```$/, ''));
    }
        ...
        const isMethodNotFound = msg.includes('-32601') || msg.includes('Method not found');
        if (!isMethodNotFound) {
            return {
...
server.prompt('generate-fake-user', 'genearete a fake user based on a given name', {
    name: z.string(),
}, ({ name }) => {
    return {
        messages: [
